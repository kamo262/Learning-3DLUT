...
void TriLinearBackwardCpu(const float* image, const float* image_grad, float* lut_grad, const int dim, const int shift, const float binsize, const int width, const int height, const int channels, const int batch)
{
    ...
    for (int batch_index = 0; batch_index < batch; ++batch_index) {
        ...
        for (int index = 0; index < output_size; ++index)
        {
            ...
            lut_grad[id000 + shift] += w000 * image_grad[batch_start_index + index + width * height];
            lut_grad[id100 + shift] += w100 * image_grad[batch_start_index + index + width * height];
            lut_grad[id010 + shift] += w010 * image_grad[batch_start_index + index + width * height];
            lut_grad[id110 + shift] += w110 * image_grad[batch_start_index + index + width * height];
            lut_grad[id001 + shift] += w001 * image_grad[batch_start_index + index + width * height];
            lut_grad[id101 + shift] += w101 * image_grad[batch_start_index + index + width * height];
            lut_grad[id011 + shift] += w011 * image_grad[batch_start_index + index + width * height];
            lut_grad[id111 + shift] += w111 * image_grad[batch_start_index + index + width * height];

            lut_grad[id000 + shift* 2] += w000 * image_grad[batch_start_index + index + width * height * 2];
            lut_grad[id100 + shift* 2] += w100 * image_grad[batch_start_index + index + width * height * 2];
            lut_grad[id010 + shift* 2] += w010 * image_grad[batch_start_index + index + width * height * 2];
            lut_grad[id110 + shift* 2] += w110 * image_grad[batch_start_index + index + width * height * 2];
            lut_grad[id001 + shift* 2] += w001 * image_grad[batch_start_index + index + width * height * 2];
            lut_grad[id101 + shift* 2] += w101 * image_grad[batch_start_index + index + width * height * 2];
            lut_grad[id011 + shift* 2] += w011 * image_grad[batch_start_index + index + width * height * 2];
            lut_grad[id111 + shift* 2] += w111 * image_grad[batch_start_index + index + width * height * 2];
        }
    }
}
