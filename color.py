...
def rgb2xyz(rgb_image, device):
    ...
    res = (
        torch.matmul(mt, temp_img.permute(1, 0, 2, 3).contiguous().view(3, -1))
        .view(3, rgb_image.size(0), rgb_image.size(2), rgb_image.size(3))
        .permute(1, 0, 2, 3)
    )
    return res
...
def cie2000_loss_image(input, target):
    input_lab = rgb2lab_diff(input.clip(0.0, 1.0), input.device)
    target_lab = rgb2lab_diff(target, input.device)
    return (
        ciede2000_diff(input_lab, target_lab, input.device)
